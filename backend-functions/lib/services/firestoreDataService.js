...
exports.getPublishedPracticeItems = getPublishedPracticeItems;
exports.createProblemSet = createProblemSet;
...
function calculateTrialDay(value) {
...
async function getDashboardForUser(token) {
...
async function getPublishedPracticeItems() {
...
async function createProblemSet(payload) {
...
async function createSubmission(token, payload) {
...
async function getSubmissionReview(token, submissionId) {
    ...
    const selectedAnswers = new Map(submission.answers.flatMap((answer) => {
        const item = asRecord(answer);
        return typeof item.questionId === 'string' && typeof item.selectedOption === 'string'
            ? [[item.questionId, item.selectedOption]]
            : [];
    }));
